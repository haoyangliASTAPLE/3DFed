...
def compute_cos_sim_loss(params: Params,
        ...
        cs_sim = F.cosine_similarity(params.fl_weight_scale*(model_vec-target_var) + target_var, target_var, dim=0)
        loss = 1e3*(1-cs_sim)
        if grads:
            grads = get_grads(params, model, loss)
        return loss, grads

def compute_noise_neup_loss(params: Params, 
                            global_dict,
                            noise_masks, 
                            random_neurons,
                            grads):
    ...
    for i in range(len(noise_masks)):
        ...
        for j in random_neurons:
            if 'MNIST' not in params.task:
                UPs.append(torch.abs( - global_dict['fc.weight'][j] + noise_masks[i].fc.weight[j]).sum() \
                    + torch.abs( - global_dict['fc.bias'][j] + noise_masks[i].fc.bias[j]))
            else:
                UPs.append(torch.abs( - global_dict['fc2.weight'][j] + noise_masks[i].fc2.weight[j]).sum() \
                    + torch.abs( - global_dict['fc2.bias'][j] + noise_masks[i].fc2.bias[j]))
        UP_loss = 0
        for j in range(len(UPs)):
            if 'Imagenet' in params.task:
                UP_loss += 5e-4 / UPs[j] # 5e-4
            elif 'Cifar' in params.task:
                UP_loss += 1e0 / (UPs[j] * params.fl_num_neurons)
            else:
                UP_loss += 1e-1 / UPs[j]
        noise_masks[i].requires_grad_(True)
        UP_loss.requires_grad_(True)
        if grads:
            grads = get_grads(params, noise_masks[i], UP_loss)
        losses.append(UP_loss)
    return losses, grads
                ...
                for j in range(layer.shape[0]):
                    if j in random_neurons:
                        sum_var[noise_size:noise_size + layer[j].view(-1).shape[0]] = \
                            (layer[j] - backdoor_update[name][j] - global_dict[name][j]).view(-1)
                    noise_size += layer[j].view(-1).shape[0]
        if 'Imagenet' in params.task:
            loss = 1e-2 * torch.norm(sum_var, p=2)
        elif 'MNIST' in params.task:
            loss = 1e-2 * torch.norm(sum_var, p=2) # 1e-2
        else:
            loss = 1e-2 * torch.norm(sum_var, p=2) # * params.fl_num_neurons
        if grads:
...
def compute_lagrange_loss(params: Params, 
    ...
    if 'Imagenet' in params.task:
        loss = 1e-2 * torch.norm(sum_var, p=2) # 1e-1
    elif 'MNIST' in params.task:
        loss = 1e-2 * torch.norm(sum_var, p=2)
    else:
        loss = 1e-2 * torch.norm(sum_var, p=2)
...
def compute_scp_acc_loss(params: Params, 
                        benign_model: Model, 
                        scapegoat: Model,
                        criterion, inputs, labels, grads):
    scp_acc_loss, _ = compute_normal_loss(params, scapegoat, criterion, inputs, labels, grads)
    benign_acc_loss, _ = compute_normal_loss(params, benign_model, criterion, inputs, labels, grads)
    if scp_acc_loss > benign_acc_loss + 3:
        loss = 0 * scp_acc_loss
        print("ERROR! Skip this dataset")
    elif scp_acc_loss > benign_acc_loss:
        loss = scp_acc_loss
    else:
        loss = - scp_acc_loss # 1e-10 * (scp_acc_loss)
    
    if grads:
        t = time.perf_counter()
        grads = list(torch.autograd.grad(loss.mean(),
                                         [x for x in scapegoat.parameters() if
                                          x.requires_grad],
                                         retain_graph=True))
        record_time(params, t, 'backward')
...
def compute_scp_param_loss(params:Params,
                        scapegoat: Model,
                        benign_model: Model,
                        param_idx,
                        grads):
    ...
    if 'MNIST' not in params.task:
        param_diff = torch.abs(scapegoat.fc.weight[param_idx[0]][param_idx[1]] - \
            benign_model.state_dict()['fc.weight'][param_idx[0]][param_idx[1]])
    else:
        param_diff = torch.abs(scapegoat.fc1.weight[param_idx[0]][param_idx[1]] - \
    ...
    if(param_diff.item() > threshold):
        ...
        loss = - 1e1 * param_diff # 1e1
    # print(param_diff.item())
    if grads:
        grads = get_grads(params, scapegoat, loss)
